...
func (j *JWTService) IssueToken(claims *JWTCustomClaims) (tokenString string, exp int64, err error) {
	...
	if claims.IssuedAt.Time.IsZero() {
		return "", 0, errors.New("invalid issued at time")
	}
...
func (j *JWTService) RefreshToken(tokenString string) (token string, err error) {
	jwt, err := j.ParseToken(tokenString)
	if err != nil {
		return "", err
	}
...
func (j *JWTService) CreateRefreshToken(claims *JWTCustomClaims) (string, error) {
	if claims == nil {
	...
	// 刷新最大时间
	maxRefreshTime := claims.IssuedAt.Time.Add(j.MaxRefreshTime)
	now := helpers.TimenowInTimezone(j.TimeLocation)
	// 检查是否超过最大刷新时间
	if now.After(maxRefreshTime) {
		return "", ErrTokenExpiredMaxRefresh
	}
	// 过期时间
	t := helpers.TimenowInTimezone(j.TimeLocation).Add(j.ExpireTime)
